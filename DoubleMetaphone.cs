...
namespace Metaphone
{
	public class MultiWordMetaphone
	{
		public DoubleMetaphone objDM;
		public int Weight = 30;
		public string Name
		{
			set
			{
				objDM = new DoubleMetaphone();
				...
				{
					string[] strWords = ms_name.Split( new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries );
					List<string> strMetaphones = new List<string>( strWords.Length );

					// calculate metaphone for each word separately
					foreach( string strWord in strWords )
					{
						objDM.Word = strWord;
						strMetaphones.Add( objDM.PrimaryKey );
					}

					// need to sort metaphone converted words to make transposed words comparable
					strMetaphones.Sort();

					// concatenate all metaphone words
					MetaphoneKey = "";
					strMetaphones.ForEach( m => MetaphoneKey = string.Concat( MetaphoneKey, " ", m ) );
					MetaphoneKey = MetaphoneKey.Trim();
				}
			}
			get
			{
	...
	public class NameSimilarity
	{
		...
		public string Name
		{
			set{
				...
				ms_name = value;
				string[] strWords = ms_name.Split( " "[0] );
				mi_value = 0;
				MetaphoneKey = "";
				// calculate metaphone for each word separately
				foreach( string strWord in strWords )
				{
					objDM.Word = strWord;
					MetaphoneKey = string.Concat( MetaphoneKey, objDM.PrimaryKey );
					string strResult = objDM.PrimaryKey;
					if( strResult.Length > 5 )
						strResult = strResult.Substring( 0, 5 );

					// calculate value for each 5 characters in the metaphone 
					// giving a higher weight to the 1st letter then less to the 2nd and so forth
					int iExponent = strResult.Length - 1;
					foreach( char objC in strResult )
					{
						mi_value += objC * Weight ^ iExponent;
						iExponent --;
					}
				}
			}
	...
	{
		public const int METAPHONE_KEY_LENGTH = 14;//The length of the metaphone keys produced.  4 is sweet spot
